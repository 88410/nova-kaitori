...
export function PageHeader({ title, backTo = '/', tone = 'light', children }: PageHeaderProps) {
    ...
    return (
      <>
        ...
        <header className="border-b border-slate-200 bg-white">
          <div className="mx-auto grid min-h-14 max-w-7xl grid-cols-[76px_minmax(0,1fr)_96px] items-center px-3 sm:grid-cols-[180px_minmax(0,1fr)_180px] sm:px-6 lg:px-8">
            <Link
              to={backTo}
              className="inline-flex min-w-0 items-center gap-2 text-sm font-medium text-slate-600 transition-colors hover:text-slate-950"
              aria-label={t('back')}
            >
              <ArrowLeft className="h-4 w-4 shrink-0" />
              <span className="truncate">{t('back')}</span>
            </Link>
            <h1 className="truncate px-2 text-center text-sm font-semibold text-slate-950 sm:text-base">{title}</h1>
  ...
  return (
    <header
      className="sticky top-0 z-40 border-b border-white/10 bg-[#07080b]"
    >
