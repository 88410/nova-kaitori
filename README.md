...
- [x] **会員セキュリティ**: サーバー側セッション、安全なCookie、CSRF保護、認証レート制限、単回パスワード再設定Token
...
- 2026-08-06: 会員認証をサーバー側セッションへ移行し、安全なCookie、CSRF検証、認証レート制限、単回・期限付きパスワード再設定Tokenを追加
- 2026-08-06: メール未設定環境ではパスワード再設定を安全に停止し、メールアドレスだけで変更できた旧APIを廃止
- 2026-08-06: FastAPI / Starlette / lxml / python-dotenv、Axios / Vite / React Router等の依存関係を更新し、認証セキュリティ試験とフロントエンドLintを追加
